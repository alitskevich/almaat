...
- `graphify-out/` — gitignored. Knowledge-graph artifacts from the `/graphify` skill (`graph.html`, `graph.json`, `GRAPH_REPORT.md`). Safe to delete and regenerate.
- `.venv/` — gitignored Python venv used by the `/graphify` tooling, not by the repo itself.

There is no test suite, no linter wired into CI, no application code.

## graphify sync on git push

A `pre-push` hook at `.git/hooks/pre-push` refreshes `graphify-out/` when the push contains changes under `docs/`:

- If `GEMINI_API_KEY` or `GOOGLE_API_KEY` is set, runs `graphify extract ./docs --backend gemini` + `cluster-only` + `export html` headlessly.
- Otherwise, touches `graphify-out/.needs_update`. Next time `/graphify` runs in a Claude session, that flag tells it to re-extract.
- The hook never blocks a push — every failure path exits 0.

It is local-only (lives in `.git/hooks/`, not tracked). When cloning fresh, re-create it or copy the version on disk. If you don't want the auto-refresh, `rm .git/hooks/pre-push`.
