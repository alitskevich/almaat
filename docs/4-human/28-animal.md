...
- species continuation, and care for offspring.

The `Animal`:

- is driven by emotions, instincts, and impulses — reactive, not proactive, responding to stimuli rather than initiating action.
- is focused on the present moment, with limited capacity for long-term planning or abstract thinking: simple over complex, familiar over unknown, quick results over delayed, predictability over success.
...
**`Emotion`** := organism arousal in response to a stimulus perceived as a threat to the `Animal`.

- Signals the presence of a threat and mobilizes energy for action.
- Can be triggered by a real or an imagined threat, and can persist after the threat is gone.
- Can be positive (joy, love) or negative (fear, anger), but all are rooted in the same underlying physiological processes.
- Intensity depends on the novelty, precision, stability, and vividness of the stimulus, and on the importance of the corresponding perception.
...
**`Fear`** := disconnection from reasoning caused by insufficient grounds for a decision in a stressful situation.

---

**`Rage`** := release of excess energy after hyper-mobilization, following a decision made by the `Animal` in a stressful situation.
