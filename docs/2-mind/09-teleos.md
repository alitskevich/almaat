...
*NOTE*: A top-level declaration of conditions to satisfy in a given Context.
...
*NOTE*: The baseline criterion for a sound `Solution` — a "success" that depletes future capability is not `Success`.
...
Whether the `Agent` authored the `Solution` at all is taken up in [Free Will](10-free-will.md).
