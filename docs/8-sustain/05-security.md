...
stateDiagram-v2
    [*] --> White
    White --> Yellow: switch on
    Yellow --> Orange: threat identified
    Orange --> Red: threat acts
    Orange --> Yellow: threat clears
    Red --> Yellow: threat ends
    Red --> Black: overwhelm
    Black --> Red: training holds
```
...
**Body language** — read clusters and shifts from baseline, not isolated signs. *Caution:* the popular claim that gaze direction reveals lying (an NLP "eye-accessing" idea) has **no empirical support** — controlled studies fail to replicate it. Treat single-cue lie detection as folklore; rely on consistency over time.
...
- Niccolò Machiavelli — [The Prince](https://en.wikipedia.org/wiki/The_Prince) (the lion and the fox). *Unreadability* adapted from pasted source material on Machiavelli's strategy of unreadability (нечитаемость); no source URL was provided.
