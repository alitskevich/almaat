...
Neither profile is the productive one: systematizing attention runs a process, scanning attention finds what the process should be. Most published productivity method is written by the first profile for the first profile, and reaches the second as evidence of being broken. A remedy whose first step already demands the capacity it is meant to build — read this book, maintain this system — cannot bootstrap.

- **Match the role, not the person.** Move the work, not the character.
- **Delegate the mismatch instead of fixing it.** Second-brain tooling is built by and for systematizing attention and earns its cost where someone is staffed to run it. Adopted alone, it becomes one more abandoned system.
- **Prefer a single, dumb capture surface.** One paper notebook beats four synced apps for a scanning profile: writing by hand encodes as it captures (see [Learning → Making It Stick](09-learning.md#making-it-stick)), with no structure to maintain or migrate.
- **Arrangement first, chemistry second.** Stimulants do work, and they also raise the cost of every day taken without them. They do not compensate for a day arranged against you.
...
For the written form of this triage, see [Brian Tracy's method](03-vision.md#questions-on-paper).
...
**Resistance boots later than you do.** For the first minute or two after waking, the deliberative system that negotiates, postpones, and objects is not yet online. Begin the hardest task inside that window and the negotiation never takes place. This is the mechanism under [eat the frog](#focus-and-priority).
...
| **Work first, eat second** | Take the first block before breakfast. See [Vitality → Nutrition](../8-sustain/01-vitality.md#6-nutrition). |
| **Hard-stop the day** | Fix the day at a length you can hold — six hours is a common ceiling. Work expands into whatever it is given; a hard edge forces the compression and exposes what should be delegated. |
| **Peaks after the stop** | Games, feeds, sugar, loud music — once the day is closed, and not before. |

## Protecting the Block

A block takes roughly twenty minutes to become productive, and any interruption costs those twenty minutes again. A day spent inside notifications never reaches the productive part at all.
